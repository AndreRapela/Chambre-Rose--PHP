...
final class ProductRepository
{
    ...
    private static function map(array $row): array
    {
        ...
        return [
            ...
            'imageSrcSet' => self::responsiveSrcSet($imageUrl),
            'secondaryImageSrcSet' => self::responsiveSrcSet($secondaryImageUrl),
    ...
    private static function responsiveSrcSet(?string $url): ?string
    {
        if ($url === null || preg_match('#^/api/products/\d+/images/(?:main|secondary)$#', $url) !== 1) {
            return null;
        }

        return ResponsiveImageService::srcSet($url);
    }
}
