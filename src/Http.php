...
final class UploadedFile
{
    ...
    {
        return $this->error === UPLOAD_ERR_NO_FILE || ($this->error === UPLOAD_ERR_OK && $this->size === 0);
    }
...
final class Request
{
    ...
    public static function fromGlobals(): self
    {
        ...
        return new self(strtoupper($_SERVER['REQUEST_METHOD'] ?? 'GET'), $path, $headers, $_GET);
    }
    ...
    public function multipart(): array
    {
        ...
        if ($contentLength > self::MAX_BODY_BYTES) {
            throw new ApiException(413, 'Multipart request cannot exceed 20MB.');
        }
        if ($this->method === 'POST') {
            $files = [];
        ...
        return $this->parsedMultipart = MultipartParser::parse($this->body(), $this->header('content-type') ?? '');
    }
    ...
    public function body(): string
    {
        if ($this->rawBody === null) {
            $contentLength = (int) ($this->header('content-length') ?? 0);
            if ($contentLength > self::MAX_BODY_BYTES) {
                throw new ApiException(413, 'Request body cannot exceed 20MB.');
            }
...
final class MultipartParser
{
    ...
    public static function parse(string $body, string $contentType): array
    {
        ...
        foreach (explode('--' . $boundary, $body) as $part) {
            ...
            $contents = preg_replace('/\r\n$/', '', substr($part, $separator + 4)) ?? '';
            if (!preg_match('/Content-Disposition:\s*form-data;([^\r\n]+)/i', $headers, $disposition)
                || !preg_match('/\bname="([^"]+)"/i', $disposition[1], $nameMatch)) {
                continue;
            }
            $name = $nameMatch[1];
            if (preg_match('/\bfilename="([^"]*)"/i', $disposition[1], $fileMatch)) {
                $type = preg_match('/Content-Type:\s*([^\r\n]+)/i', $headers, $typeMatch)
                    ? trim($typeMatch[1]) : 'application/octet-stream';
                $fileName = trim(str_replace('"', '', basename(str_replace('\\', '/', $fileMatch[1])))) ?: 'product-image';
...
final class Response
{
    ...
    {
        $reasons = [400 => 'Bad Request', 401 => 'Unauthorized', 403 => 'Forbidden', 404 => 'Not Found',
